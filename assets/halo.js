...
(function ($) {
  ...
  var ella = {
    ...
    init: function () {
      ...
      if(body.hasClass('template-index') || body.hasClass('template-page') || body.hasClass('best-offers-on-plum')) {
        this.initSlideshow();
      ...
      $(document).ready((function(){$("label.cstm-in_label").click((function(t){console.log("check"),t.preventDefault();var r=$(this).attr("data-variant"),d=$(this).closest("form.variants").attr("data-product");$(this).closest("form.variants").find(".qty-group.newtab").hasClass("variant-"+r)?($(this).closest(".products-grid").find("form.variants[data-product="+d+"] .qty-group.newtab").hide(),$(this).closest(".products-grid").find("form.variants[data-product="+d+"] .cart").hide(),$(this).closest(".products-grid").find("form.variants[data-product="+d+"] .qty-group.newtab.variant-"+r).show()):($(this).closest(".products-grid").find("form.variants[data-product="+d+"] .cart").show(),$(this).closest(".products-grid").find("form.variants[data-product="+d+"] .qty-group.newtab").hide()),$(this).closest(".srting_it").find("input").prop("checked",!1).removeAttr("checked"),$(this).closest(".products-grid").find("input[value="+r+"]").attr("checked",!0).prop("checked",!0);var i=$(this).data("price"),a=$(this).data("compare"),o=$(this).data("image");"0"!=a?$(this).closest(".products-grid").find("form.variants[data-product="+d+"] .old-price .money").text("₹ "+a):$(this).closest(".products-grid").find("form.variants[data-product="+d+"] .old-price .money").text(""),$(this).closest(".products-grid").find("form.variants[data-product="+d+"] .special-price .money").text("₹ "+i),$(this).closest(".products-grid").find(".product-item[data-product-id=product-"+d+"] .product-grid-image .images-one").attr("src",o),$(this).closest(".products-grid").find(".product-item[data-product-id=product-"+d+"] .product-grid-image .images-one").attr("data-src",o),$(this).closest(".products-grid").find(".product-item[data-product-id=product-"+d+"] .product-grid-image .images-one").attr("data-srcie",o),$(this).closest(".products-grid").find(".product-item[data-product-id=product-"+d+"] .product-grid-image .images-one").attr("data-srcief",o);var s=$(this).closest(".products-grid").find(".product-item[data-product-id=product-"+d+"] .product-grid-image").attr("href");$(this).closest(".products-grid").find('.product-item[data-product-id="product-'+d+'"] .product-grid-image').attr("href",s+"?variant="+r),$(this).closest(".products-grid").find('.product-item[data-product-id="product-'+d+'"] .product-bottom .product-title').attr("href",s+"?variant="+r)}))}));
...
initBlogPostSlider: function() {
   ...
   kitAddtoCart:function (){
      // On kit Submit
        $('form#add-to-cart-form-kit').submit(function(e){
             e.preventDefault();
             var selectedvalues = $(this);
             var variantIdArray = [];
             let mainproduct = selectedvalues.find('#product_id').val();
             // selected variants 
             $(selectedvalues).find("input[type='radio']:checked").each(function(index,value){
                  console.log(this);
                  variantIdArray[index] = {
                      'quantity': 1,
                      'id': $(this).val(),
                  }
             });
             console.log(variantIdArray);
             return ella.addAllItems(variantIdArray);

        });
    },
    addAllItems: function (variantIdArray) {
        // add the items to the data object we need to pass to the fetch call 
        var data = {
          items: variantIdArray
        }
         $('#kit-product-add-to-cart').val('adding..');              
        fetch('/cart/add.js', {
            ...
            ella.updateDropdownCart();
            setTimeout(function () {
                   $('#kit-product-add-to-cart').val('Add to cart');
            }, 3000);             
          }).catch((err) => {
          /* uh oh, we have error. */
            console.error(err)
